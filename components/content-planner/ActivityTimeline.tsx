...
// Exact activity types the planningActivity mapper produces.
type EventType =
  | "CREATED"
  | "EDITED"
  | "FAILED"
...
// Typed icon map — every key maps to a valid LucideIcon export.
// Unknown/catch-all events fall back to Activity so the component never renders
// an undefined <Icon />.
...
function safeType(raw: PlanningActivity["type"] | undefined | null): EventType {
  ...
  // Fallback for null/undefined/unknown types — never crash.
  return "FAILED";
}
...
export default function ActivityTimeline({ items }: { items: PlanningActivity[] }) {
  const sorted = [...items]
    ...
    .sort(
      (a, b) =>
        (Number.isFinite(new Date(b.at).getTime())
    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 backdrop-blur-xl">
      ...
      <div className="relative space-y-3 pl-4">
        ...
        {sorted.map((a, i) => {
          ...
          const tint =
            type === "PUBLISHED" || type === "APPROVED"
              ? "text-emerald-300 bg-emerald-400/10"
              : type === "REJECTED" || type === "FAILED"
          ...
          return (
            ...
            >
              <span
                className={`absolute -left-[19px] flex h-5 w-5 items-center justify-center rounded-full ${tint}`}
              >
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="text-xs text-white/75">
                  ...
                  {" "}
                  {verbFor(type)} {a.contentTitle && <span className="text-white/55">“{a.contentTitle}”</span>}
                </div>
