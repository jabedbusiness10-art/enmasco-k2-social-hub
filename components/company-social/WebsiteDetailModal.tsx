...
export default function WebsiteDetailModal({
  ...
  const rows: [string, string][] = [
    ...
    ["Authentication", conn.authMethod],
    ["Publishing", conn.publishingEnabled ? "Enabled" : "Read only"],
    ["Status", conn.status],
    ["Health", conn.health],
    ...
    ["Last Publish", conn.lastPublish ? new Date(conn.lastPublish).toLocaleString() : "Never"],
    ["Last Webhook", conn.lastWebhook ? new Date(conn.lastWebhook).toLocaleString() : "Never"],
    ["Last Error", conn.lastError ?? "None"],
