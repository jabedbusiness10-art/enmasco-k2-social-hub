    ...
    <div className="space-y-4 p-4 text-sm text-white/80">
      ...
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {["MD Kazim", "Lipton", "Arif", "Sumon", "Rahman"].map((name, i) => (
            <span key={`${name}-${i}`} className="rounded-full border border-white/10 bg-white/[0.05] px-2 py-1 text-xs text-white/80">
              {name}
            </span>
          ))}
        </div>
